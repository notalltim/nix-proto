{ lib }: rec {
  /**
    ...
    **NOTE**: The name given to this function must match the one given to `generateOverlays'`
    ...
    ```
    mkProtoDerivation :: AttrSet -> (AttrSet -> Derivation)
    ```
  ...
  */
  ...
  mkProtoDerivation = set: { stdenvNoCC }: stdenvNoCC.mkDerivation (set // {
  ...
  */
  generateOverlays' = set:
    let
      inherit (lib.attrsets) mapAttrs' nameValuePair;
    in
    mapAttrs' (name: drv: nameValuePair (name + "_overlay") (generateOverlay' { inherit drv; inherit name; })) set;
  ...
  */
  ...
  /**
    ...
    The function maps the attibute stet of derivation to a set of overlays called with a specific `callPackage` signature
    ...
    ```
    generateOverlay' :: AttrSet -> (PkgSet -> PkgSet -> Package)
    ```
