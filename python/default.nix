{lib}: rec {
  ...
  inherit (lib.strings) escapeShellArg concatMapStringsSep concatStringsSep concatStrings;
  inherit (lib) recursiveProtoDeps toProtocInclude loadMeta nixProtoWarn;
  ...
  */
  patchImports = name: fullProtoDeps: ''
    ...
    declare -A deps=${escapeShellArg ("(" + (concatMapStringsSep " " (dep: "[\"" + dep.name + "\"]" + "=\"" + dep.src + "\"") fullProtoDeps) + ")")}
  ...
  */
  grpc = {
    ...
    dependencies = [protobuf grpcio] ++ (toBuildDepsPy (protoMeta.protoDeps ++ [protoMeta]) python.pkgs); # Pull python deps from the current python package set

    # PyProjectTOML file for generated code
    py_project_toml = toPyProjectTOML {
      inherit name;
      inherit (protoMeta) version;
      dependencies = ["protobuf" "grpcio"] ++ toPythonDependencies (protoMeta.protoDeps ++ [protoMeta]);
      pythonVersion = python.pythonVersion;
    };
  ...
  in
    buildPythonPackage {
      ...
      nativeBuildInputs = [grpcio-tools setuptools]; # for import checking
      propagatedBuildInputs = dependencies; # This is needed because `dependencies` only works on unstable right now
  ...
  */
  package = {
    ...
    pkgs.python3Packages.buildPythonPackage rec {
      ...
      postPatch = (patchImports name (recursiveProtoDeps proto_meta.protoDeps ++ [proto_meta])) + py_project_toml;
    };
  ...
  */
  ...
  protobufLegacy = {
    ...
    nixProtoWarn "Deprecated: use the protobuf derivation it is built to support `pythonPackagesExtensions` by taking python has an argument"
    package rec {
      inherit pkgs;
